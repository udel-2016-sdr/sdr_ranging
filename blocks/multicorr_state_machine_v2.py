...
class multicorr_state_machine_v2(gr.sync_block):
    """
    Multicorrelator State Machine (v2) - Controls delay of reference signal to maximize
                                         correlation
    """
    ...
    def __init__(self, dly_cont_name='', max_delay=0):
        gr.sync_block.__init__(
            self,
            "Multicorrelator State Machine (v2)",
            in_sig=[(np.float32, 3)],
            out_sig=[(np.int32, 3)]
        )
